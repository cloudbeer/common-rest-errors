...
var errors = {
    ...
    WHAT_TIMEOUT: function (what) {
        what = what || '对象';
        return YError(what + "已经超时。", -1011);

    },
    WHAT_EXPIRED: function (what) {
        what = what || '对象';
        return YError(what + "已过期。", -1012);
    },
    WHAT_ILLEGAL: function (what) {
        what = what || '对象';
        return YError(what + "不合法。", -1013);
    },
    WHAT_NOT_FOUND: function (what) {
        what = what || '对象';
        return YError(what + "不存在。", -1404);
    },
    STRING_LENGTH:function (what, min, max) {
        what = what || '参数';
        var msg = what + "长度错误。";
        if (min) {
            msg += "，最小 " + min;
